...
class Efficient_Attention_Gate(nn.Module):
    ...
    def forward(self, g, x):
        ...
        out = x * psi if x is not None else g1 * psi
        out += x if x is not None else g1
...
class EFF(nn.Module):
    def __init__(self, in_dim, skip_dim, is_bottom=False):
        super().__init__()
        self.is_bottom = is_bottom
        if not is_bottom:
            self.EAG = Efficient_Attention_Gate(F_g=in_dim, F_l=skip_dim, F_int=max(in_dim, skip_dim))
        else:
